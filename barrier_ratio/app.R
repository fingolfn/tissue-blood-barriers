...
plot_tissueMap<-function(drug,gene,data){
  ...
  if(length(row_idx)==1){
    ...
    subplot(color.bar(rbPal(100),-2,title = "Tissue:Blood Ratio"), x=-50, y=700,size=c(0.2,5))
    
    #ADD Model Key:
    tmp_col<-c("black","black","black","black")
...
barrier_csv<-read.csv("barriers.csv",as.is=T)
...
gene_list_4list<-c("WT","abcb1a -/-","abcg2 -/-","TKO")
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(width=3,
                 selectInput(inputId = 'search',label = 'Select Drug Below',choices = drug_names_4list,selected="Digoxin_4hr"),
                 radioButtons(inputId = 'gene',label = 'Genetics',choices = gene_list_4list,selected = "WT"),
